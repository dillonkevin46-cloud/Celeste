...
class TodoView(ctk.CTkFrame):
    ...
    def refresh_grid(self):
        ...
        for task in tasks:
            ...
            ctk.CTkCheckBox(row_frame, text="", variable=select_var, width=50).pack(side="left", padx=10, pady=5)
            ...
            # Status Checkbox (Instant Toggle)
            ...
            ctk.CTkCheckBox(row_frame, text="Done", variable=status_var, width=100, command=make_toggle(task_id, status_var)).pack(side="left", padx=10)
    ...
    def open_task_dialog(self):
        ...
        def save_task():
            ...
            database.add_task(name, assignee, priority, due, "Pending")
            dialog.destroy()
            self.refresh_grid()
...
class KpiView(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.btn_add_kpi.pack(side="left")
    ...
    def build_kpi_grid(self):
        ...
        for kpi in kpi_data:
            ...
            btn_edit = ctk.CTkButton(actions_sub, text="Edit", width=50, command=lambda tid=task_id, tname=task_name: self.edit_kpi_dialog(tid, tname))
            btn_edit.pack(side="left", padx=2)

            btn_del = ctk.CTkButton(actions_sub, text="Del", width=50, fg_color="#C8504B", hover_color="#8E3533", command=lambda tid=task_id: self.delete_kpi(tid))
    ...
    def draw_chart(self):
        ...
        # Color based on theme (assuming dark mode for matplotlib patch)
        bar_color = '#3B8ED0'
        bars = self.ax.bar(days, percentages, color=bar_color)

        # Target line
        self.ax.axhline(y=75, color='#C8504B', linestyle='-', linewidth=2, label='Target 75%')
...
class SettingsView(ctk.CTkFrame):
    ...
    def build_user_management(self):
        ...
        ctk.CTkButton(self.user_frame, text="+ Add User", command=lambda: self.user_dialog()).pack(pady=15)
    ...
    def build_assignee_management(self):
        ...
        ctk.CTkButton(self.assignee_frame, text="+ Add Assignee", command=lambda: self.assignee_dialog()).pack(pady=15)
